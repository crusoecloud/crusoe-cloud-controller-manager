...
const (
	...
	nicClusterID  = "0c8f5c85-c9ab-4f0f-9a4c-9a3b6d3c25a1"
)
...
// (section 9): no lister, no SDN, just cfg + the API client and the NIC cache.
...
func TestResolveNIC_DoesNotMutateConfig(t *testing.T) {
	...
	// cfg is immutable after startup (section 5.1): a mismatching instance only warns,
...
func TestResolveLocationFromCluster(t *testing.T) {
	...
	ctrl := gomock.NewController(t)
	m := mock_client.NewMockApiClient(ctrl)
	m.EXPECT().GetClusterByID(gomock.Any(), nicProjectID, nicClusterID).
		Return(&crusoeapi.KubernetesCluster{Id: nicClusterID, Location: nicLocation}, nil)

	loc, err := ResolveLocationFromCluster(context.Background(), m, nicProjectID, nicClusterID)
	if err != nil {
...
func TestResolveLocationFromCluster_LookupFailurePropagates(t *testing.T) {
	...
	// The error propagates; startRouteController treats it as fatal (section 5.1).
	if _, err := ResolveLocationFromCluster(context.Background(), m, nicProjectID, nicClusterID); err == nil {
		t.Fatalf("expected error to propagate")
	}
}

func TestResolveClusterCIDRFromVPC(t *testing.T) {
	t.Parallel()
	ctrl := gomock.NewController(t)
	m := mock_client.NewMockApiClient(ctrl)
	m.EXPECT().GetVPCNetworkByID(gomock.Any(), nicProjectID, nicVPCID).
		Return(&crusoeapi.VpcNetwork{Id: nicVPCID, Cidr: "121.0.0.0/20"}, nil)

	cidr, err := ResolveClusterCIDRFromVPC(context.Background(), m, nicProjectID, nicVPCID)
	if err != nil {
		t.Fatalf("resolveClusterCIDRFromVPC: %v", err)
	}
	if cidr != "121.0.0.0/20" {
		t.Fatalf("expected vpc cidr, got %s", cidr)
	}
}

func TestResolveClusterCIDRFromVPC_LookupFailurePropagates(t *testing.T) {
	t.Parallel()
	ctrl := gomock.NewController(t)
	m := mock_client.NewMockApiClient(ctrl)
	m.EXPECT().GetVPCNetworkByID(gomock.Any(), nicProjectID, nicVPCID).
	...
	// The error propagates; DefaultClusterCIDR treats it as fatal.
	if _, err := ResolveClusterCIDRFromVPC(context.Background(), m, nicProjectID, nicVPCID); err == nil {
		t.Fatalf("expected error to propagate")
	}
}
