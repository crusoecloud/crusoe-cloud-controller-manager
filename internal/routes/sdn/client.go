...
type PodCIDRAllocationClient interface {
	...
	// ListPodCIDRAllocationOperations supports repeated operation_ids. CreateRoute
	// polls one operation at a time.
	...
	// ResolveLocation maps a location's external (customer-facing) name, as the
	// Crusoe API reports it on clusters and instances, to the internal name the
	// region coordinator requires in every PodCIDRAllocationContext. Called once
	// at startup. An input that already is an internal name resolves to itself.
	ResolveLocation(ctx context.Context, externalName string) (string, error)
}
