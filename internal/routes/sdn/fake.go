...
type LoggingFakeClient struct {
	...
	// its row for the idempotent retry). The flag auto-clears. Test knob.
	FailNext bool
...
// ResolveLocation is the identity: the fake has no location table, so the
// external name is used as-is in the allocation context.
func (f *LoggingFakeClient) ResolveLocation(_ context.Context, externalName string) (string, error) {
...
// pre-existing allocation (e.g. the CloudRoutes ListRoutes paths).
...
func (f *LoggingFakeClient) ListPodCIDRAllocations(
	...
	// Server parity (validateListPodCIDRAllocationsQuery in region-coordinator):
	// every List requires project id AND vpc network id; all other filters are
	// optional and intersect.
	if q.ProjectID == "" || q.VPCNetworkID == "" {
		return nil, ErrInvalidArgument
	}
...
func (f *LoggingFakeClient) advance(fo *fakeOp) {
	...
	if fo.fail {
		...
		// row in place (the idempotent retry converges on that same row), and a
		// failed delete retains its rows. So a FAILED op mutates nothing here.
		return
	}
