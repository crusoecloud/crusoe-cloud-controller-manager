...
func TestCreate_HappyPath(t *testing.T) {
	...
	allocs, err := f.ListPodCIDRAllocations(context.Background(),
		sdn.ListPodCIDRAllocationsQuery{ProjectID: testProject, VPCNetworkID: testVPC, DestinationCIDR: testCIDR})
	if err != nil {
		t.Fatalf("list: %v", err)
	}
	if len(allocs) != 1 {
...
func TestDelete_RemovesRowOnSuccess(t *testing.T) {
	...
	allocs := mustList(t, f,
		sdn.ListPodCIDRAllocationsQuery{ProjectID: testProject, VPCNetworkID: testVPC, PodCIDRAllocationIDs: []string{id}})
	if len(allocs) != 1 {
...
func TestList_FilterIntersectionAndOrdering(t *testing.T) {
	...
	all, err := f.ListPodCIDRAllocations(context.Background(),
		sdn.ListPodCIDRAllocationsQuery{
			ProjectID: testProject, VPCNetworkID: testVPC,
			VPCPrefixReservationIDs: []string{testReservation},
		})
	if err != nil {
		t.Fatalf("list: %v", err)
	}
	...
	one, err := f.ListPodCIDRAllocations(context.Background(),
		sdn.ListPodCIDRAllocationsQuery{
			ProjectID: testProject, VPCNetworkID: testVPC,
			VPCPrefixReservationIDs: []string{testReservation}, NetworkInterfaceID: "nic-b",
		})
	if err != nil {
