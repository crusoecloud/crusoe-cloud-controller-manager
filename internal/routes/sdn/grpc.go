...
type GRPCClient struct {
	...
	status       region.RegionStatusClient
}
...
func NewGRPCClient(endpoint, certFile, keyFile, caFile string) (*GRPCClient, error) {
	...
	return &GRPCClient{
		...
		status:       region.NewRegionStatusClient(conn),
	}, nil
}
...
// ResolveLocation lists the active locations and returns the internal name of
// the one whose external name (or internal name) matches.
func (c *GRPCClient) ResolveLocation(ctx context.Context, externalName string) (string, error) {
	resp, err := c.status.ListLocations(ctx, &region.ListLocationsRequest{})
	if err != nil {
		return "", mapError(err)
	}

	return internalLocationName(resp.GetLocations(), externalName)
}

// internalLocationName picks the location matching externalName by external
// name first, then by internal name (so an already-internal input is a no-op).
func internalLocationName(locs []*component.Location, externalName string) (string, error) {
	known := make([]string, 0, len(locs))
	for _, l := range locs {
		if l.GetExternalName() == externalName || l.GetName() == externalName {
			return l.GetName(), nil
		}
		known = append(known, fmt.Sprintf("%s (%s)", l.GetName(), l.GetExternalName()))
	}

	return "", fmt.Errorf("%w: location %q not among active locations [%s]",
		ErrNotFound, externalName, strings.Join(known, ", "))
}
