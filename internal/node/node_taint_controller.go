...
const (
	TaintLabelPrefix        = "crusoe.aai/taint."
	ManagedTaintKeyPrefix   = "crusoe.aai/"
	NodePoolIDLabel         = "crusoe.ai/nodepool.id"
	NodeTaintControllerName = "node-taint-controller"
)

// TaintController applies taints to Kubernetes nodes based on nodepool labels
...
// NewTaintController creates a new TaintController.
...
func (c *TaintController) Run(ctx context.Context,
...
func (c *TaintController) syncTaints(ctx context.Context) {
	...
	for _, node := range nodes {
		if err := c.reconcileNodeTaints(node); err != nil {
...
func (c *TaintController) reconcileNodeTaints(node *v1.Node) error { //nolint:unparam // error kept for future use
	...
	if cached, ok := c.nodepoolTaints.Load(nodepoolID); ok {
		desiredTaints, _ = cached.([]v1.Taint) //nolint:errcheck // type assertion ok checked
	}
...
func ParseTaintsFromLabels(nodeLabels map[string]string) []v1.Taint {
	var taints []v1.Taint //nolint:prealloc // size unknown until filtering
