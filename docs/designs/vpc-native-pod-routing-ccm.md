...
implementation over the existing `sdn` seam. The revision-(g) analysis (old section 19)
...
git history (see section 16). Sections kept from the previous design because they still
hold: the `sdn` seam (section 4), configuration (section 5), NIC resolution (section 9), the real
gRPC client (section 17).
...
- **`routes.PodCIDRMirror`**, a write-once controller copying the CiliumNode v4
  ...
  populates them and the upstream controller reads nothing else (section 7).
- **Wiring** in `internal/cloud.go` so `Cloud.Routes()` returns the impl in
  native mode and `(nil, false)` otherwise (section 8).
...
- Allocation deletion on node departure beyond what the upstream delete loop
  ...
  `DeleteRoute` surfaces that error until the server lands it (section 6.3).
...
- Per-node happy path: Node registers → mirror seeds
  `NodeNetworkUnavailable=True` (section 11) → KCM adds the standard
  `node.kubernetes.io/network-unavailable` taint → cilium writes
  `CiliumNode.spec.ipam.podCIDRs` → mirror copies the v4 /24 to
  `node.spec.podCIDR`+`podCIDRs` (write-once) → next upstream pass sees the
  node with a podCIDR and no matching route → `CreateRoute` (synchronous:
  create allocation + poll op to terminal) → upstream sets
  `NodeNetworkUnavailable=False` → KCM removes the taint.
- Orphan GC: an allocation whose NIC maps to no live node is returned from
  ...
  (`--cluster-cidr` defaults to the VPC CIDR, section 10): ownership is enforced by `ListRoutes`
  returning only our configured reservations' allocations.
- Unresolvable NIC: `ListRoutes` logs it and attributes that node's
  allocations by `spec.podCIDRs` instead, so the pass continues and a silently
  dropped node does not make its allocation look orphaned.
...
```
                 Node add ──► PodCIDRMirror seeds NodeNetworkUnavailable=True
                              (KCM taints node.kubernetes.io/network-unavailable)
                                        │
CiliumNode (cilium cluster-pool)        ▼
  spec.ipam.podCIDRs ──► PodCIDRMirror (this repo, section 7) ──► node.spec.podCIDR/podCIDRs
                            write-once patch                        │
                                                                    ▼
                          ...
                          every --route-reconciliation-period (10s):
                            ...
                            └► NodeNetworkUnavailable condition on Node
                                        │                        │
                                        ▼                        ▼
                          CloudRoutes (this repo, section 6)    KCM node-lifecycle ctrl
                            sdn seam + NIC resolution      condition ⇒ taint add/remove
                                        │
                                        ▼
                          island.v2.region.PodCIDRAllocationManagement (gRPC, section 17)
```
...
2. `cloud.Routes()` returns `(impl, true)`, only in native mode (section 8).
...
moment `Routes()` returns non-nil on a cluster without a parseable
`--cluster-cidr`, CCM startup dies. Two guards close it: `Routes()` returns
`(nil, false)` unless the native config loads successfully (section 8), and in
native mode `doInitializer` defaults an unset `--cluster-cidr` to the VPC
network's CIDR fetched at startup (section 10), so the deployment does not
have to render the flag at all.
...
type PodCIDRAllocationClient interface {
	...
	// CloudRoutes.DeleteRoute (section 6.3). Server-side still Unimplemented.
...
Error taxonomy (sentinels in client.go, mapped reason-based by grpc.go, section 17):
...
`CRUSOE_SDN_USE_FAKE=true` opts into it (local dev only, section 5).
...
cluster object via `--cluster-name`, which carries the Crusoe cluster UUID,
so the lookup is `GetClusterByID` (`internal/routes/location.go`, fatal on
failure, cfg immutable afterwards). That is the **external** location name
(`us-southcentraldevelopment1-a`); the region coordinator accepts **internal**
names only (`txdr-iah-dev`), so after the SDN client is built the name is
mapped once via `RegionStatus.ListLocations` (`ExternalName` → `Name`,
`sdn.PodCIDRAllocationClient.ResolveLocation`) into `Config.SDNLocation`,
which is what every `PodCIDRAllocationContext` carries. `Config.Location`
keeps the external name for the instance-metadata sanity check
(`warnMetadataMismatch`). No match among active locations is fatal. SDN
client selection: native mode needs
exactly one of `REGION_ENDPOINT` (⇒ gRPC, mTLS iff the
`CRUSOE_SDN_CERT_FILE`/`_KEY_FILE`/`_CA_FILE` trio is set; the name matches
addon-controller MR 68, which renders the cmk-addon-catalog ConfigMap's
`regionEndpoint` key, a bare host such as `10.71.1.1`, so a value without a
port gets the region coordinator's gRPC port `9090` appended) and
`CRUSOE_SDN_USE_FAKE=true` (⇒ the in-memory logging fake, an explicit local-dev
opt-in, never a silent fallback). `ErrInconsistentSDNConfig` on neither or both,
or on a partial trio. Overlay mode ignores the SDN vars entirely: they are
region-level wiring the deployment may render regardless of routing mode, and
nothing dials the SDN in overlay mode.
...
becomes package consts (section 6.2):
...
1. List all nodes from `nodeLister`. Build `nicID → nodeName` by resolving
   every live node's NIC (section 9, cached; **steady-state zero API calls**).
   Cache entries for node names the lister no longer returns are evicted first.
   A node whose NIC does not resolve is logged and its `spec.podCIDRs` are
   recorded in a `cidr → nodeName` fallback map instead of failing the pass:
   the allocation is still attributed to the node (step 3), so it is protected
   from orphan GC without an API call, and the NIC check that matters happens
   in `CreateRoute`.
...
3. Map each allocation → `&cloudprovider.Route{Name: alloc.ID,
   TargetNode: types.NodeName(nicToNode[alloc.NetworkInterfaceID])`, falling
   back to `cidrToNode[alloc.DestinationCIDR]` when the NIC is unknown,
   `DestinationCIDR: alloc.DestinationCIDR}`. An allocation matching neither
   gets `TargetNode: ""` → the upstream pass **deletes** it. That is
...
1. `node, err := nodeLister.Get(string(route.TargetNode))`, upstream built the
   ...
   next pass).
2. `nicID := resolveNIC(node)` (section 9). Error → error.
3. **List-before-create / adopt**:
   `ListPodCIDRAllocations(DestinationCIDR: route.DestinationCIDR,
   VPCPrefixReservationIDs: cfg.VPCPrefixReservationIDs)`:
   - found with **our** NIC → adopt, return nil. A route programmed once is
     never re-created, whatever drives the pass; the SDN write path is not
     touched. (Also covers crash-between-create-and-poll, op-retention gaps,
     and FAILED creates whose row survived.)
   - found with a **different** NIC → return the wrapped
     ...
     race self-resolves because VM deletion cascades its allocations
     server-side (agreement with the SDN team); once server-side Delete works,
     the orphan-GC path (section 6.1) is the backstop.
   - absent → continue.
4. Pick the reservation by cidr containment, `reservationForCIDR` relocated
   ...
   `ListVPCPrefixReservations` + `netip` containment. Reservations are
   immutable, so the fetched set is cached on `CloudRoutes` after the first
   lookup.
5. `CreatePodCIDRAllocations` (1 spec, context `{ProjectID, VPCID, SDNLocation}`).
...
op, err := r.sdn.DeletePodCIDRAllocations(ctx, sdn.DeletePodCIDRAllocationsRequest{
	IDs:     []string{route.Name}, // allocation id from ListRoutes (section 6.1)
...
Crusoe API client and location wiring move to `internal/cloud.go` (section 8); the
...
- Reconcile(name):
  ...
  3. Get CiliumNode from the lister, convert via `ciliumNodeFromUnstructured`
     (section  below); pick the **first v4** prefix from `spec.ipam.podCIDRs`
...
section 12, harmless while server-side Delete is Unimplemented, and the mirror must
...
func (c *Cloud) Initialize(clientBuilder cloudprovider.ControllerClientBuilder, stop <-chan struct{}) {
	...
	// location, fail-fast (section 5): resolveLocationFromCluster(ctx, c.apiClient,
	// cfg.ProjectID, c.clusterName); err → klog.Fatalf (crash-loop is the retry)
	// SDN client per section 5/section 17 (fake only on explicit opt-in); Close wired to stop:
	//   go func() { <-stop; closeSDN() }()
	// cfg.SDNLocation, err = sdnClient.ResolveLocation(ctx, cfg.Location); err → klog.Fatalf
...
- `buildSDNClient` and the location resolution move from `register.go` into
  ...
  retains only the mirror wiring (section 7). `buildAPIClient` duplication disappears:
...
Rendered on **native clusters only**, alongside the existing env (section 5):

| Flag | Value | Why |
|---|---|---|
| `--cluster-cidr` | **optional; leave unset** | In native mode the CCM defaults it to the VPC network's CIDR, fetched at startup via `GetVPCNetworkByID` (`Cloud.DefaultClusterCIDR`, injected into the CompletedConfig from `doInitializer` before controllers start; lookup failure is fatal). An explicitly set flag wins. The upstream controller uses the value only to scope **deletions** (`isResponsibleForRoute`); VPC prefix reservations are carved from the VPC range, so the VPC CIDR covers every reservation, including ones added out-of-band after a mis-sized create. No lockstep updates ever needed. Max one v4 if set (two only as a dual-stack pair, core.go:121-129). |
...
`CRUSOE_VPC_PREFIX_RESERVATION_IDS` / `CRUSOE_SDN_*`, except that the fake is
now an explicit `CRUSOE_SDN_USE_FAKE=true` opt-in, not an endpoint-absent
fallback (section 5).
...
- **Bootstrap window, closed by the mirror**: nothing else provides the
  initial condition. The kubelet sets none for external providers, the
  upstream controller (v0.35.5 `reconcile`) skips a node with no podCIDR
  before creating its routeMap entry and then skips the status update for
  nodes without one, and KCM's `doNoScheduleTaintingPass` treats
  `network-unavailable` as condition-owned and strips a kubelet-registered
  taint whenever the condition is not `True`. So `PodCIDRMirror.reconcile`
  seeds `NodeNetworkUnavailable=True`/`NoRouteCreated` on any node that has no
  such condition (Node add handler, same `nodeutil.SetNodeCondition` helper
  and reason as upstream); KCM taints immediately and upstream flips it to
  `False` after `CreateRoute`. An existing condition is never overridden. A
  kubelet `--register-with-taints` is harmless but no longer needed.
- **Cilium must not write the condition** (cross-repo ask, replaces the kubelet
  taint ask): the cilium operator's default `--set-cilium-is-up-condition`
  sets `NodeNetworkUnavailable=False`/`CiliumIsUp` once the agent is ready,
  which would lift the taint before the route exists (observed on the dev
  cluster). Native clusters must render `operator.setNodeNetworkStatus: false`.
- **First rollout on a cluster with existing nodes**: nodes carrying no
  condition get tainted for at most one pass (10s) until the upstream
  controller adopts their allocation and writes `False`.
...
| NIC resolution fails for any live node | Logged; that node's allocations are attributed by its `spec.podCIDRs` instead, so the pass continues and the allocation is not orphan-GC'd. A node with no podCIDR yet has nothing to protect. |
| Allocation whose NIC maps to no live node | `TargetNode: ""` ⇒ upstream deletes (orphan GC). Ownership scoping comes from `ListRoutes`' reservation-id filter; `--cluster-cidr` defaults to the VPC CIDR (section 10). Inert until server-side Delete lands. |
...
| Poll deadline (2m) exceeded | Error; op continues server-side; next pass adopts. |
...
- **Logs**: upstream route controller logs every create/delete/condition action
  ...
  would-be RPC (section 4); mirror logs each write-once patch.
...
Deployment env: unchanged (section 5) except that native mode now requires
`REGION_ENDPOINT` to be rendered (or `CRUSOE_SDN_USE_FAKE=true` for local
dev), there is no endpoint-absent fallback any more. New addon-controller ask:
render `--cluster-cidr` per section 10 on native clusters. The current release manifest binds
...
|---|---|---|
| `CloudRoutes` | fake sdn client (`sdn.NewLoggingFakeClient` + knobs) + gomock `APIClient` + node lister from `cache.NewIndexer` (no informers, no envtest) | ListRoutes: allocation→node mapping via NIC; unresolvable NIC → pass survives, allocation attributed by pod cidr; allocation with unknown NIC → `TargetNode: ""`; cache eviction for nodes gone from the lister; steady-state zero API calls (mock call count with warm cache); cache re-resolve when node's instance id changes. CreateRoute: happy path (create + poll to SUCCEEDED via `PendingPolls`); adopt pre-seeded row (ours) without create; conflict (other NIC / `ConflictCIDRs`) → error; `FailNext` → error, then next call adopts the surviving row; poll timeout → error; multi-reservation containment pick with one reservation list RPC across two creates. DeleteRoute: id passthrough (`route.Name`), Unimplemented error surfaced, no op polling. |
...
replaced by the upstream route controller + section 6/section 7 of this revision. The full
...
(`DeletePodCIDRAllocations` not yet served, section 6.3). `Operation.AllocationIDs`
from `BulkOperationMetadata.resource_ids`. Client selection per section 5
(`CRUSOE_SDN_USE_FAKE=true` ⇒ logging fake, else dial `REGION_ENDPOINT`).
Building requires
`GOPRIVATE=gitlab.com/crusoeenergy/*`. The only change in this revision is the
call site: the conn's `Close` is wired to `Initialize`'s stop channel (section 8)
instead of the deleted register.go context hook.

## 18. Open Questions / Risks

- **Pod-cidr fallback attribution**: a node whose NIC cannot be resolved keeps
  its allocation only while `node.spec.podCIDRs` still carries the cidr. A node
  that is both unresolvable and podCIDR-less (mirror lag plus an API outage)
  would see its allocation read as an orphan. Judged acceptable against the
  alternative, one unresolvable node stalling reconciliation cluster-wide.
- **No grace period / delete cap anymore**: once server-side Delete lands, a
  ...
  (upstream has no cap). Reservation-id scoping plus `--cluster-cidr` are the
  defense; re-evaluate
...
- **Blocking CreateRoute under SDN degradation**: a pass stalls up to
  `createPollTimeout` (2m) on the slowest create (passes are serialized).
...
- **Mirror-before-server-Delete rollout gate** (section 12 migration note) must be
  tracked in the addon-controller/kubernetes-manager sequencing.
- **`--cluster-cidr` / reservation drift**: eliminated by contract -
  `--cluster-cidr` defaults to the VPC CIDR at startup (section 10), so deletion scoping never
  ...
  expansion runbook, section 12).
- **Cilium operator condition write** (section 11): the mirror closes the
  condition-absent window, but the gate only holds if the cilium operator runs
  with `--set-cilium-is-up-condition=false` on native clusters. Open until the
  cilium rendering for native clusters carries that setting.
...
| 2 | `internal/routes`: CloudRoutes, `cloudprovider.Routes` over the sdn seam | New `routes.go` (section 6: ListRoutes mapping, synchronous CreateRoute with poll, DeleteRoute passthrough; NIC cache per section 9) + `routes_test.go` (section 15 row 1). Make `resolveInstance`/`instanceByID` standalone funcs in `nic.go` (parameterized on `client.APIClient`+`Config`) so both the old controller (untouched, still compiling) and `CloudRoutes` use them; relocate `reservationForCIDR` to a standalone func (new `reservation.go`), old call site delegates. Exported and deliberately unwired, `Cloud.Routes()` still returns `(nil,false)`. | done: 843a884 |
| 3 | `internal/routes`: PodCIDRMirror controller | New `mirror.go` + `mirror_test.go` (section 7, section 15 row 2). Exported, unwired (registration happens in commit 4). Uses the existing `ciliumNodeFromUnstructured`/GVR. | done: bc24cc3 |
| 4 | the swap: wire `Cloud.Routes()`, register the mirror, delete the custom controller | `internal/cloud.go`: `apiClient`/`clusterName`/`routes` fields, `Initialize` native construction (fatal on failure, SDN Close on stop), `SetInformers`, `Routes()` (section 8); `main.go`: `SetClusterName` in `doInitializer`, map key → `"crusoe-podcidr-mirror"`. `register.go` rewritten to start only the mirror (section 7). DELETE: `controller.go`, `optracker.go`, `reaper.go`, `reconcile.go`, `reconcile_helpers.go`, `nodepatch.go`, `metrics.go`, `helpers_test.go` + their tests. Shrink `ciliumnode.go` projection to Name+PodCIDRs (+test). Trim `Config` knobs (section 5) + config tests. Update the `DeletePodCIDRAllocations` seam comment (section 4). | done: 7a99804 |
...
| 6 | `internal/routes`: review follow-ups (cidr fallback, explicit fake) | `createPollTimeout` 5m -> 2m; `ListRoutes` no longer fails the pass on an unresolvable NIC (pod-cidr fallback attribution) and evicts `nicByNode` entries for nodes gone from the lister; cache the VPC prefix reservations on `CloudRoutes` (immutable, one list RPC); `CRUSOE_SDN_USE_FAKE` makes the logging fake an explicit opt-in instead of an endpoint-absent fallback; 30s timeout on the startup Crusoe API lookups in `cloud.go`; mirror clients built from `controllerContext.ClientBuilder` so `ClientName` is honored. Tests updated in `routes_test.go` / `config_test.go`. `CreateRoute`'s list-before-create/adopt step was briefly dropped in favor of contract idempotency and then reinstated: adopting keeps a once-programmed route off the SDN write path regardless of what drives the pass. | done (this commit) |
...
- **`ResolveLocationFromCluster` / `BuildSDNClient` exported:** section 8 said SDN + location wiring move into `internal/cloud.go` "exported from the routes package as needed". Location resolution became exported (`ResolveLocationFromCluster`) and `buildSDNClient` became `routes.BuildSDNClient` in a new `wiring.go`; `cloud.go` (package `crusoe`) calls both. `buildAPIClient` was dropped, `newCloud`'s client is retained on the `Cloud` struct per section 8.
...
- **PodCIDRMirror tests use an indexer-backed `GenericLister`, not `dynamicfake`** (section 15 row 2 mentioned dynamicfake). An indexer lister drives `reconcile` directly with fewer moving parts; the mirror reads CiliumNodes only through `cache.GenericLister`, so the double is equivalent.
...
- **Stale `sdn` doc comments:** the `opTracker`/`reaper` mentions left in `sdn/{types,client}.go` by commit 4 were scrubbed in commit 6.
- **No `.golangci.yml` change:** the config had no per-file excludes for the deleted files, so there was nothing to remove.
- **SDN vars in overlay mode:** commit 6 rejected any SDN var in overlay mode as a misrender. That crashed CCMs on clusters where the deployment renders the region-level SDN endpoint and certs for every cluster; `validateOverlay` now checks only the VPC vars and SDN vars are validated in native mode alone.
- **Commit 6, reservation cache:** cached on `CloudRoutes` under the existing `mu` and only consulted when more than one reservation id is configured (the single-id path still short-circuits without any RPC). A failed list is not cached.
